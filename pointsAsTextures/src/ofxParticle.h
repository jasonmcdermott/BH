...
public:
    ...
    ofVec3f pos, vel, acc, ali, coh, sep, att, origin, bias, outerTemp, other, dist, circleLocation, centre;
    ...
    string string;
    bool reset, isDead, avoidBoundaries, outside, inside, orbit, gravitate;
    bool flocking;
    ...
    void initParticle() {
        float offset = 5;
        isDead == false;
        origin.set(centre.x + ofRandom(-offset,offset), centre.y + ofRandom(-offset,offset), centre.z + ofRandom(-offset,offset));
        pos.set(origin);
        vel.set(0,0,0);
        acc.set(0,0,0);
        bias.set(0,0,0);
        ...
        shapeType = 0;
        reset = false;
        torusInnerRadius = 100;
        torusOuterRadius = 200;
        innerBoundF = 100.0;
        outerBoundF = 100.0;
    }
        
    void run(vector <ofxParticle> particles) {
        if (reset == true) {
            initParticle();
        }
        if (isDead != true) {
...
//
    ...
    void moveInward() {
        ...
        ofVec3f futureLocation(pos);
        futureLocation += predict;
        ofVec3f distFromCircleCentre(centre);
        distFromCircleCentre -= futureLocation;
        float distance = distFromCircleCentre.length();
